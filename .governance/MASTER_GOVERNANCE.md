...
- **Deploy and entry modes:** `docs/DEPLOY.md` (ES modules vs Webpack bundle, git/Vercel, audit posture); **security posture:** `docs/SECURITY.md`; **integrations (Supabase, APIs, Cloudinary, EmailJS, admin vs storefront):** `docs/INTEGRATIONS.md`; **Supabase fork (RLS):** `docs/SUPABASE_FORK.md`
...
| **Dev server** | Default: Node `scripts/static-dev-server.mjs` (`npm run dev`). Optional Python: `scripts/server.py` (`npm run dev:python`), `scripts/server_secure.py` — **not** the Vercel production runtime unless replaced with serverless |
